...
namespace Crystal
{
    ...
    public class SafeArea : MonoBehaviour
    {
        ...
        Vector2Int[] DeviceSize =
        {
            new Vector2Int(1920, 1080), // default
            new Vector2Int(2436, 1125), //iPhoneX
            new Vector2Int(2688, 1242), //iPhoneXsMax
            new Vector2Int(2960, 1440), //Pixel3XL_LSL
            new Vector2Int(2960, 1440),  //Pixel3XL_LSR
            new Vector2Int(2778, 1284),  //iPhone 12 Pro Max
        };
        ...
        private SimDevice GetSim()
        {
            for (int i = 0; i < DeviceSize.Length; i++)
            {
                //Debug.Log($"({Screen.height} == {DeviceSize[i].x} || {Screen.width} == {DeviceSize[i].x}) && ({Screen.height} == {DeviceSize[i].y} || {Screen.width} == {DeviceSize[i].y})");
                if ((Screen.height == DeviceSize[i].x || Screen.width == DeviceSize[i].x) && (Screen.height == DeviceSize[i].y || Screen.width == DeviceSize[i].y))
                {
                    return (SimDevice)i;
                }
            }
