...
public class BetterButtonEditor : OdinEditor
{
    private InspectorProperty _onClick;
    ...
    private static List<BetterButtonEditor> s_Editors = new List<BetterButtonEditor>();
    ...
    {
        _tree = this.Tree;
        _onClick = _tree.GetPropertyAtPath("_onClick");
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (graphic == null)
            graphic = ((Selectable) target).GetComponent<Graphic>();

        var animator = ((Selectable) target).GetComponent<Animator>();
        m_ShowColorTint.target = (!m_TransitionProperty.hasMultipleDifferentValues && trans == Selectable.Transition.ColorTint);
        m_ShowSpriteTrasition.target = (!m_TransitionProperty.hasMultipleDifferentValues && trans == Selectable.Transition.SpriteSwap);
        m_ShowAnimTransition.target = (!m_TransitionProperty.hasMultipleDifferentValues && trans == Selectable.Transition.Animation);
        ...
        ++EditorGUI.indentLevel;
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(m_ShowAnimTransition.faded))
            {
                ...
                if (animator == null || animator.runtimeAnimatorController == null)
                {
                    ...
                    {
                        var controller = GenerateSelectableAnimatorContoller(((Selectable) target).animationTriggers, target as Selectable);
                        if (controller != null)
                        {
                            if (animator == null)
                                animator = ((Selectable) target)?.gameObject.AddComponent<Animator>();
