...
public class FPSDisplay : MonoBehaviour {
	...
	static string[] stringsFrom00To199 = {
	...
	[SerializeField]
	private FPSColor[] coloring = default;
	...
	void Update ()
	{
		if (!isFPSPanelActive.v) return;
		Display(highestFPSLabel, highFPS.v);
		Display(averageFPSLabel, averageFPS.v);
		Display(lowestFPSLabel, lowFPS.v);
	}

	void Display (TMP_Text label, int fps) {
		label.text = stringsFrom00To199[Mathf.Clamp(fps, 0, 199)];
