...
public class PlayerWeaponsManager : MonoBehaviour
{
    ...
    public bool AddWeapon(WeaponController weaponPrefab, bool isUnlocked)
    {
        ...
        for (int i = 0; i < m_WeaponSlots.Length; i++)
        {
            ...
            if(m_WeaponSlots[i] == null)
            {
                ...
                // Set ammo amount text if there is one
                if (i < currentAmmoTexts.Length)
                {
                    weaponInstance.CurrentAmmoText = currentAmmoTexts[i];
                    if (isUnlocked) weaponInstance.CurrentAmmoText.text = Mathf.RoundToInt(weaponInstance.startAmmo).ToString();
                }
                ...
                weaponInstance.isWeaponUnlocked = isUnlocked;
                m_WeaponSlots[i] = weaponInstance;

                onAddedWeapon?.Invoke(weaponInstance, i);

                //Debug.Log(weaponInstance.weaponName + " added and is " + (isUnlocked ? "unlocked" : "locked"));
                return true;
            }
            if (!m_WeaponSlots[i].isWeaponUnlocked)
            {
                m_WeaponSlots[i].isWeaponUnlocked = true;
                Debug.Log(m_WeaponSlots[i].weaponName + " unlocked");
                var currentAmmoText = m_WeaponSlots[i].CurrentAmmoText;
                if (currentAmmoText != null)
                    currentAmmoText.text = Mathf.RoundToInt(m_WeaponSlots[i].startAmmo).ToString();
                onAddedWeapon?.Invoke(m_WeaponSlots[i], i);
                return true;
            }
        }
    ...
    public WeaponController GetWeaponAtSlotIndex(int index)
    {
        ...
        {
            if (m_WeaponSlots[index] == null) return null;
            return m_WeaponSlots[index].isWeaponUnlocked ? m_WeaponSlots[index] : null;
        }
    ...
    void OnWeaponSwitched(WeaponController newWeapon)
    {
        if (newWeapon != null)
        {
            ...
            weaponUIManager.OnWeaponSwitched(newWeapon.weaponName);
        }
    }
}
