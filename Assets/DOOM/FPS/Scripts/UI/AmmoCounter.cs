...
public class AmmoCounter : MonoBehaviour
{
    ...
    public float changeAnimSpeed = 10;
    ...
    void Update()
    {
        ...
        ammoText.text = Mathf.RoundToInt(m_Weapon.CurrentAmmo).ToString();
        bool isActiveWeapon = m_Weapon == m_PlayerWeaponsManager.GetActiveWeapon();
        
        canvasGroup.alpha = Mathf.Lerp(canvasGroup.alpha , isActiveWeapon ? 1f : unselectedOpacity, Time.deltaTime * changeAnimSpeed);
        transform.localScale = Vector3.Lerp(transform.localScale, isActiveWeapon ? Vector3.one : unselectedScale, Time.deltaTime * changeAnimSpeed);
