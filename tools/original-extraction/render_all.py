...
# 0x61 controls signed integer amplitude arithmetic, not floating point gain.
def amplify(x, level):
    x -= 128
    if level == 1: x -= x >> 1
    elif level == 2: x -= x >> 2
...
def delay_of(p):
    signed = (CC2*2+p+128) % 256 - 128
    return max(0, 0x10-signed)
def interpret(cmds):
    out=bytearray(); i=0
    amp=0; delay=DELAY0
    def emit(seg):
        ...
        if op==0x20: out+=b'\x80'*151; i+=1    # dbra 0x245e ~= 11.6ms @8MHz
            ...
            if V in VARSEL:
                ...
                if 0<=si<644 and 0<=ei<644 and clipoff[ei]<=len(audio):
                    emit(audio[clipoff[si]:max(clipoff[si]+1,clipoff[ei])])
            else: raise ValueError(f'Unsupported speech variant: {V}')
        elif op==0x04:
            ...
            if 0<=idx<643 and clipoff[idx+1]<=len(audio):
                emit(audio[clipoff[idx]:max(clipoff[idx]+1,clipoff[idx+1])])
        else: raise ValueError(f'Unsupported speech command: {op}')
    return out
def render(i):
